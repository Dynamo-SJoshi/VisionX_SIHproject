...
## Core Rules for M2
1. **Pipeline**: `Frame` → `Object Detection (YOLO)` → `Pose/Hands (MediaPipe)` → `Tracker (ByteTrack/BoT-SORT)` → `Spatial Context (ArUco)` → `Hand-Object Interaction` → `Temporal Action Buffer` → `ActionEvent`.
...
4. **Edge First**: Keep models lightweight (YOLOv8n/11n), CPU/hardware-agnostic, and offline-ready. Do not depend on CUDA/Jetson-only APIs.
5. **Boundaries**: Do NOT build protocol state machines, UI, FastAPI endpoints, TTS, or alert logic.
